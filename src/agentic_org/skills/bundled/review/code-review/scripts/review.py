...
def run(
    ...
    review_neighborhood: dict[str, Any] | None = None
    if graph_dir:
        try:
            from agentic_org.code_graph import load_graph, review_pack

            review_neighborhood = review_pack(load_graph(graph_dir), paths)
            if review_neighborhood.get("inferred_count"):
                findings.append({
                    "severity": "warn",
                    "code": "graph_inferred_edges",
                    "detail": (
                        f"{review_neighborhood['inferred_count']} review-pack "
                        "file(s) reached via INFERRED edges — not certain"
                    ),
                })
            ...
            findings.append({
                ...
                "detail": f"code graph unused: {type(exc).__name__}: {exc}",
            })
    ...
    return {
        ...
        "changed_paths": paths,
        "review_pack": review_neighborhood,
        "evidence": _EVIDENCE,
    }
